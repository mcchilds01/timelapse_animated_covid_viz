...
def initial_setup(source='owid-covid-data.csv', file_format='csv'):
	"""
	
	Allows user to select source format for initial setup; source can take either a url or csv filepath, with the csv as default. 
	file_format parameter can take either 'json' or 'csv' as an argument, with 'csv' as default. 
	
	"""
	if file_format == 'json':
		initial_setup_from_json(source)
	else: initial_setup_from_csv(source)
...
def initial_setup_from_csv(source):
	"""

	Creates the initial 180-day plot, after which only daily updates are needed. 

	"""
	covid_data = pd.read_csv(source)
...
def plot_map(covid_rates_list, date):
	...
	wm.title=f'Total COVID cases by country as of {months[str(date)[5:7]]} {str(date)[0:4]}'
...
def convert_to_gif():
	...
	imageio.mimsave(f'GIS_project/COVID_viz/COVID_gif_{date.today()}.gif', images, duration = 0.5)
